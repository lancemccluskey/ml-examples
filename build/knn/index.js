...
const { features, labels, testFeatures, testLabels } = convertFeaturesToTensor(data, ['horsepower', 'weight', 'mpg'], ['passedemissions'], 25);
const classifier = knnClassifier.create();
console.log(labels.length);
console.log(features.shape[0]);
for (let row = 0; row < features.shape[0]; row++) {
    // example: tensor, label: number|string
    const featureRow = features.slice([row, 0], // 0 index of first row
    [1, -1] // 1 row, whole row
    );
...
for (let row = 0; row < testFeatures.shape[0]; row++) {
    const testFeatureRow = testFeatures.slice([row, 0], [1, -1]);
    // input: tensor, k: number
    const prediction = await classifier.predictClass(testFeatureRow);
    if (parseInt(prediction.label) === testLabels[row][0])
        accuracy.correct++;
    else
        accuracy.incorrect++;
}
